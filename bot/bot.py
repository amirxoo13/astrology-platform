    ...
    if step == 'name':
        ...
        try:
            if ',' in text:
                ...
                state['data']['location_name'] = f"{lat}, {lon}"
            else:
                # Use Tehran as default
                state['data']['latitude'] = 35.69
                state['data']['longitude'] = 51.39
                state['data']['location_name'] = text
        except:
            state['data']['latitude'] = 35.69
            state['data']['longitude'] = 51.39
            state['data']['location_name'] = text
...
async def generate_and_send_chart(query, context):
    ...
    # Call API
    chart_data = call_api('/api/v1/birth-chart', birth_data)
    aspects_data = call_api('/api/v1/aspects', birth_data)
